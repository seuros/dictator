...
fn check_mod_rs_structure(path: &str, diags: &mut Diagnostics) {
    ...
    if sibling_count == 0 {
        ...
        diags.push(Diagnostic {
            ...
            message: format!(
                "mod.rs with no submodules should be {module_name}.rs - refactor when you need it, inshallah"
            ),
            enforced: false,
            span: Span::new(0, 100),
        });
...
impl Decree for RustDecree {
    ...
    fn lint(&self, path: &str, source: &str) -> Diagnostics {
        ...
        check_mod_rs_structure(path, &mut diags);
